...
(() => {
  ...
  var CONFIG = {
    ...
    api: {
      ...
      maxTokens: 2048,
      // floor; scaled up with tab count
      maxTokensCeiling: 8192,
      tokensPerTab: 24,
      // max_tokens grows by this per tab...
      tokensBuffer: 256,
      // ...plus this fixed buffer
      temperature: 0,
      // deterministic → repeatable clustering
      seed: 7,
      // best-effort reproducibility; ignored by
      // providers/models that don't support it
      timeoutMs: 9e4,
      // abort a hung request
      errorBodyMaxChars: 300,
      // truncate HTTP error bodies in logs
      outputPreviewMaxChars: 200,
      // truncate unparseable model output in logs
    ...
    ui: {
      ...
      // SETTINGS-1: spec-locked literal
      notificationValue: "zen-tidy-tabs-msg",
      // TIDY-14: spec-locked literal
      label: "🧹 Tidy",
      busyLabel: "↻ Tidying…",
    ...
    panel: {
      ...
      // PANEL-3: hide the redundant "Save and close group"
      overrideUngroup: true,
      // PANEL-4: run "Ungroup tabs" ourselves (Zen's is inert)
      ids: {
    ...
    grouping: {
      ...
      minTabs: 3,
      // refuse to tidy fewer than this
      minGroups: 2,
      // lower bound for the model's group cap
      maxGroups: 8,
      // upper bound for the model's group cap
      targetTabsPerGroup: 3
      // group cap scales as ceil(tabCount / this)
    },
    snapshot: {
      titleMax: 160,
      // truncate long titles before sending to the model
      urlMax: 120
      // truncate long URLs too
    },
    // Timing constants in ms (counts where noted) for debounces, polling, retries.
    timing: {
      emptyCheckDelayMs: 80,
      // delay before the first empty-group sweep
      emptyCheckIntervalMs: 150,
      // interval between empty-group sweeps
      emptyCheckMaxTries: 6,
      // number of empty-group sweeps per schedule
      emptyWatcherDebounceMs: 500,
      // debounce for the drag-driven empty watcher
      notifyDurationMs: 6e3,
      // how long a notification stays up
      mountRetryMs: 250,
      // interval between mount attempts
      mountMaxAttempts: 40
      // give up mounting after this many attempts
    }
  };
  var PREFIX = "[Zen Tidy Tabs]";
  var makeLogger = (stage) => {
  ...
  if (!env) {
    Log.init.error(
      "Startup aborted: no window with gBrowser was found. Run this in the Browser Console (Ctrl+Shift+J) with devtools.chrome.enabled = true — the page web console (F12) cannot access the browser chrome."
    );
    throw new Error();
  }
  var { win, doc, gBrowser } = env;
  var LOAD_TOKEN = {};
  var TAB_SELECTOR = "tab, .tabbrowser-tab";
  var normalizeName = (name) => String(name ?? "").trim().toLowerCase();
  var getGroupName = (el) => (el?.label || el?.getAttribute?.("label") || "").trim();
  var getGroupColor = (el) => el?.color || el?.getAttribute?.("color") || "";
  var getGroupTabs = (el) => el.tabs || el.querySelectorAll(TAB_SELECTOR);
  var setGroupName = (el, name) => {
    el.label = name;
    el.setAttribute("label", name);
  };
  var setGroupColor = (el, color) => {
    el.color = color;
    el.setAttribute("color", color);
  };
  var CLEAR_SELECTOR = "toolbarbutton, button, label, span, hbox, vbox, toolbaritem, div, image, [label], [tooltiptext]";
  var matchesClear = (el) => {
  ...
  var dom = {
    ...
    activeWorkspaceEl() {
      return win.gZenWorkspaces?.activeWorkspaceElement || doc.querySelector("zen-workspace[active]") || gBrowser.selectedTab?.closest?.("zen-workspace") || doc.querySelector("zen-workspace");
    },
    ...
    activeSection() {
      return gBrowser.selectedTab?.closest?.(".zen-workspace-tabs-section") || doc.querySelector(".zen-workspace-tabs-section[active]") || doc.querySelector(".zen-workspace-tabs-section");
    },
...
${JSON.stringify(snapshot)}
    ...
    async post(body, apiKey) {
      ...
      try {
        ...
        if (e?.name === "AbortError") {
          Log.ai.error(
            `OpenRouter request aborted after exceeding the ${CONFIG.api.timeoutMs / 1e3}s timeout (model: ${body.model}).`
          );
          throw new Error(
            `OpenRouter request timed out after ${CONFIG.api.timeoutMs / 1e3}s`
          );
        }
        Log.ai.error(
    ...
    extractText(data) {
      ...
      if (data.choices?.[0]?.finish_reason === "length") {
        ...
        throw new Error(
          "Model response was truncated before completing the JSON (hit the output token limit). Try tidying fewer tabs or use a model with a larger output budget."
        );
      }
      ...
      if (!content) {
        ...
        throw new Error(
          "Model returned empty content. Try a concrete instruct model (e.g. openai/gpt-4o-mini) instead of a free/reasoning router."
        );
      }
    ...
    parseGroups(text, sourceTabs) {
      ...
      for (const group of groupList) {
        ...
        for (const raw of Array.isArray(group?.tabs) ? group.tabs : []) {
          ...
          if (typeof index !== "number" || !Number.isInteger(index) || index < 0 || index >= sourceTabs.length || used.has(index))
            continue;
          used.add(index);
  ...
  var groups = {
    ...
    reconcile(plan, existing) {
      ...
      const nextColor = () => {
        for (let i = 0; i < palette.length; i++) {
          const color2 = palette[paletteIndex++ % palette.length];
          if (!usedColors.has(color2)) {
    ...
    detachAndDissolve(el, stage) {
      ...
      if (groups.hasLiveTabs(el)) return members.length;
      try {
  ...
  var make = {
    ...
    field(labelText, control2) {
      const field = make.el("div", "zen-tidy-tabs-field");
      field.append(make.el("label", "zen-tidy-tabs-label", labelText), control2);
      return field;
    },
  ...
  var control = {
    ...
    twinIsCurrent() {
      ...
      return !!clear && existing.parentElement === clear.parentElement && existing.nextElementSibling === clear;
    },
    ...
    installClearWatcher() {
      ...
      const handler = () => {
        const existing = doc.getElementById(CONFIG.ui.controlId);
        if (existing?.dataset?.twin === "1" && existing.isConnected && existing.nextElementSibling && matchesClear(existing.nextElementSibling) && dom.activeWorkspaceEl()?.contains(existing))
          return;
  ...
  var editor = {
    active: null,
    // { input, labelEl, group, original, discard, cleanup }
    install() {
  ...
  var diag = {
    ...
    newTabButton() {
      return doc.getElementById("tabs-newtab-button") || doc.querySelector(
        "[command='cmd_newNavigatorTab'], .tabs-newtab-button, #vertical-tabs-newtab-button"
      ) || [...doc.querySelectorAll("toolbarbutton, button")].find(
        (b) => /new tab/i.test(
    ...
    run() {
      ...
      if (newTab?.parentElement) {
        Log.diagnose.info(
          "  newTab siblings:",
          [...newTab.parentElement.children].map((c) => dom.describe(c)).join("  |  ")
        );
        Log.diagnose.info(
          "  newTab parent path:",
