...
${JSON.stringify(e)}
    ...
    `},inject(){let e=O.theme;l.getElementById(n.ui.styleId)?.remove();let t=l.createElement("style");t.id=n.ui.styleId,t.textContent=`
      #${n.ui.controlId} {
        cursor: pointer;
      ...
      ${O.labelStyleCss()}
    `,(l.head||l.documentElement).appendChild(t),i.styles.debug(`Stylesheet injected (#${n.ui.styleId}, labelStyle: ${h.labelStyle()}).`)}};var _={segmentedControl(e,t,o){let r=t,a=y.el("div","zen-tidy-tabs-segment");return e.forEach(([s,d])=>{let c=y.el("button","zen-tidy-tabs-seg",d);s===t&&c.classList.add("active"),c.addEventListener("click",()=>{r=s,a.querySelectorAll(".zen-tidy-tabs-seg").forEach(u=>{u.classList.remove("active")}),c.classList.add("active"),o?.(s)}),a.append(c)}),{el:a,get:()=>r}},settings(){let{body:e,footer:t}=C.open("Zen Tidy Tabs Settings");i.user.debug("Opened the settings modal.");let o=y.input(h.apiKey(),{type:"password",placeholder:"sk-or-v1-..."}),r=y.input(h.model(),{placeholder:n.api.defaultModel}),a=_.segmentedControl([["filled","Colored"],["text","Text only"]],h.labelStyle()),s={detailed:"The tab's title and full URL are sent to the AI.",compact:"The tab's title and hostname are sent to the AI.",minimal:"Only the tab's title is sent to the AI."},d=y.el("p","zen-tidy-tabs-privacy-note",s[h.urlMode()]??s.detailed),c=_.segmentedControl([["detailed","Detailed"],["compact","Compact"],["minimal","Minimal"]],h.urlMode(),T=>{d.textContent=s[T]??s.detailed}),u=y.el("p","zen-tidy-tabs-hint");u.append(l.createTextNode("Key is stored locally. Get one at "));let m="https://openrouter.ai/keys",w=y.el("a","zen-tidy-tabs-link","openrouter.ai/keys");w.href=m;let x=T=>{T?.preventDefault(),C.close(),typeof g.openTrustedLinkIn=="function"?g.openTrustedLinkIn(m,"tab"):typeof p.addTrustedTab=="function"?p.selectedTab=p.addTrustedTab(m):i.user.error(`Could not open ${m}: no trusted-link API is available in this build.`)};w.addEventListener("click",x),w.addEventListener("keydown",T=>{T.key===" "&&x(T)}),u.append(w,l.createTextNode(".")),e.append(y.field("OpenRouter API key",o),y.field("Model",r),y.field("Group labels",a.el),y.field("Tab info sent to AI",c.el),d,u);let E=y.button("Cancel","ghost");E.addEventListener("click",C.close);let v=y.button("Save settings","primary");v.addEventListener("click",()=>{h.set(n.prefs.apiKey,o.value.trim()),h.set(n.prefs.model,r.value.trim()),h.set(n.prefs.labelStyle,a.get()),h.set(n.prefs.urlMode,c.get()),i.user.info(`Settings saved (model: ${r.value.trim()||n.api.defaultModel}, labelStyle: ${a.get()}, urlMode: ${c.get()}, apiKey: ${o.value.trim()?"set":"empty"}).`),O.inject(),C.close(),z.notify("Settings saved.")}),t.append(y.el("div","zen-tidy-tabs-spacer"),E,v),o.focus()}};var $={build(e){let t=l.createElement(e?e.tagName:"span");t.id=n.ui.controlId,t.textContent=n.ui.label,t.setAttribute("label",n.ui.label),t.setAttribute("tooltiptext",n.ui.tooltip),t.title=n.ui.tooltip,t.className=e?e.className:"zen-tidy-tabs-fallback",e&&(t.classList.remove(n.ui.clearButtonClass),t.dataset.twin="1");let o=r=>{r.preventDefault(),r.stopPropagation(),i.user.debug("Tidy control activated (click / command)."),z.runTidy()};return t.addEventListener("click",o),t.addEventListener("command",o),t.addEventListener("contextmenu",r=>{r.preventDefault(),r.stopPropagation(),i.user.debug("Tidy control right-clicked; opening settings."),_.settings()}),t},twinIsCurrent(){let e=l.getElementById(n.ui.controlId);if(!(e?.dataset?.twin==="1"&&e.isConnected))return!1;let t=b.clearControl();return!!t&&e.parentElement===t.parentElement&&e.nextElementSibling===t},placeTwinIfClearPresent(){if($.twinIsCurrent())return!0;let e=b.clearControl();return e?.parentElement?(l.getElementById(n.ui.controlId)?.remove(),e.parentElement.insertBefore($.build(e),e),i.dom.info(`Tidy control mounted as a twin of the Clear button (${b.describe(e)}).`),!0):!1},installClearWatcher(){let e=g.__zenTidyTabsClearWatcher;if(e?.token===B)return;e&&e.target.removeEventListener("mouseover",e.handler,!0);let t=l.documentElement,o=()=>{let r=l.getElementById(n.ui.controlId);r?.dataset?.twin==="1"&&r.isConnected&&r.nextElementSibling&&D(r.nextElementSibling)&&b.activeWorkspaceEl()?.contains(r)||$.placeTwinIfClearPresent()};t.addEventListener("mouseover",o,!0),g.__zenTidyTabsClearWatcher={token:B,target:t,handler:o},i.dom.debug("Clear-button hover watcher installed on",`${b.describe(t)}.`)},installWorkspaceWatcher(){let e=g.__zenTidyTabsWorkspaceWatcher;if(e?.token===B)return;let t=g.gZenWorkspaces;if(typeof t?.addChangeListeners!="function")return;e&&t.removeChangeListeners?.(e.listener);let o=()=>$.mount();t.addChangeListeners(o,{once:!1}),g.__zenTidyTabsWorkspaceWatcher={token:B,listener:o},i.dom.debug("Workspace-change watcher installed; Tidy control will follow the active workspace.")},mount(){if($.installClearWatcher(),$.installWorkspaceWatcher(),$.placeTwinIfClearPresent())return!0;let e=l.getElementById(n.ui.controlId),t=b.activeSection();if(e&&t&&!t.contains(e)&&e.remove(),!l.getElementById(n.ui.controlId)){let o=t&&b.firstNormalNode(t);if(o?.parentElement)return o.parentElement.insertBefore($.build(null),o),i.dom.info("Tidy control mounted via separator fallback (hover to reveal; will upgrade to a Clear twin when one appears)."),!0}return i.dom.debug("No mount target available yet; will retry or wait for a hover."),!!l.getElementById(n.ui.controlId)},setBusy(e){let t=l.getElementById(n.ui.controlId);t&&(t.textContent=e?n.ui.busyLabel:n.ui.label,t.setAttribute("label",e?n.ui.busyLabel:n.ui.label),t.style.pointerEvents=e?"none":"")}};var z={running:!1,notify(e,t=!1){(t?i.tidy.error:i.tidy.info)(e);try{let o=p.getNotificationBox(),r=o.appendNotification(n.ui.notificationValue,{label:`Zen Tidy Tabs: ${e}`,priority:t?o.PRIORITY_WARNING_HIGH:o.PRIORITY_INFO_LOW},[]);Promise.resolve(r).then(a=>{a&&setTimeout(()=>{try{o.removeNotification(a)}catch{}},n.timing.notifyDurationMs)})}catch{}},async runTidy(){if(z.running){i.tidy.debug("Ignoring Tidy request: a tidy run is already in progress.");return}let e=h.apiKey();if(!e){i.tidy.warn("Tidy aborted: no OpenRouter API key configured."),z.notify(`Set your key in about:config → ${n.prefs.apiKey}`,!0);return}let t=I.collect(!0);if(t.length<n.grouping.minTabs){i.tidy.warn(`Tidy aborted: only ${t.length} eligible tab(s), need at least ${n.grouping.minTabs}.`),z.notify(`Need at least ${n.grouping.minTabs} tabs to tidy.`,!0);return}z.running=!0,$.setBusy(!0);try{i.tidy.info(`Starting tidy of ${t.length} tab(s) (model: ${h.model()}, urlMode: ${h.urlMode()}).`);let o=await M.request(I.snapshot(t),e,h.model()),r=M.parseGroups(M.extractText(o),t);i.tidy.info("Grouping plan:",r.map(d=>`${d.name}(${d.tabs.length})`).join(", "));let{realized:a,failed:s}=k.apply(r);k.scheduleEmptyCheck(),s===0?(i.tidy.info(`Tidy complete: sorted ${t.length} tab(s) into ${a} group(s).`),z.notify(`Sorted ${t.length} tabs into ${a} groups.`)):a>0?(i.tidy.warn(`Tidy partially complete: created ${a} group(s), ${s} could not be created.`),z.notify(`Sorted ${t.length} tabs into ${a} groups; ${s} could not be created.`,!0)):(i.tidy.error(`Tidy failed: none of the ${r.length} group(s) could be created.`),z.notify("Tidy failed: no groups could be created.",!0))}catch(o){i.tidy.error("Tidy run failed.",o),z.notify(`Tidy failed: ${o.message||o}`,!0)}finally{z.running=!1,$.setBusy(!1)}}};var G={customize(){n.panel.hideSaveAndClose&&G.hideSaveAndClose(),n.panel.overrideUngroup&&G.installUngroupOverride()},hideSaveAndClose(){let e=l.getElementById(n.panel.ids.saveAndClose);e&&(e.hidden=!0)},installUngroupOverride(){if(g.__zenTidyTabsPanelOverride)return;let e=p.tabGroupMenu?.panel;if(!e)return;let t=o=>{o.target?.id===n.panel.ids.ungroup&&(o.preventDefault(),o.stopPropagation(),G.ungroup(p.tabGroupMenu?.activeGroup))};e.addEventListener("command",t,!0),g.__zenTidyTabsPanelOverride={panel:e,onCommand:t},i.user.debug("Installed 'Ungroup tabs' override on the native panel.")},uninstall(){let e=g.__zenTidyTabsPanelOverride;if(e){try{e.panel.removeEventListener("command",e.onCommand,!0)}catch{}g.__zenTidyTabsPanelOverride=null}},ungroup(e){if(!e)return;let t=L(e),o=k.detachAndDissolve(e,`ungrouping "${t}"`);try{p.tabGroupMenu?.close?.()}catch{}i.user.info(`Ungrouped ${o} tab(s) from "${t}".`)}};var J="http://www.w3.org/1999/xhtml",S={active:null,install(){let e=g.__zenTidyTabsEditorListeners;e&&(l.removeEventListener("click",e.onClick,!0),l.removeEventListener("contextmenu",e.onContextMenu,!0)),S.cancelInline(),l.querySelectorAll(".zen-tidy-tabs-inline-input").forEach(r=>{let a=r.previousElementSibling;a?.tagName?.toLowerCase()==="span"&&a.remove(),r.remove()}),l.querySelectorAll(".zen-tidy-tabs-inline-editing").forEach(r=>{r.style.removeProperty("display"),r.classList.remove("zen-tidy-tabs-inline-editing")}),l.documentElement.classList.remove("zen-tidy-tabs-editing"),G.uninstall();let t=r=>{if(r.button!==0)return;if(r.target?.closest?.(".zen-tidy-tabs-inline-input")){r.stopPropagation();return}let a=r.target?.closest?.(".tab-group-label");if(!a)return;let s=a.closest("tab-group");s&&(r.preventDefault(),r.stopPropagation(),S.startInline(s,a))},o=r=>{let a=r.target?.closest?.(".tab-group-label, .zen-tidy-tabs-inline-input");if(!a)return;let s=a.closest("tab-group");s&&(r.preventDefault(),r.stopPropagation(),S.cancelInline(),setTimeout(()=>{try{p.tabGroupMenu?.openEditModal(s),G.customize()}catch(d){i.user.error("Failed to open Zen's native group edit panel.",d)}},0))};l.addEventListener("click",t,!0),l.addEventListener("contextmenu",o,!0),g.__zenTidyTabsEditorListeners={onClick:t,onContextMenu:o},i.user.debug("Group label editor installed.")},startInline(e,t){if(S.active?.labelEl===t){S.active.input.focus();return}S.cancelInline();let o=L(e),r=l.createElementNS(J,"input");r.className="zen-tidy-tabs-inline-input",r.value=o,r.setAttribute("aria-label","Rename group");let a=getComputedStyle(t);["fontFamily","fontSize","fontWeight","fontStyle","letterSpacing","lineHeight","color","backgroundColor","backgroundImage","paddingTop","paddingRight","paddingBottom","paddingLeft","borderRadius","height","textAlign","textShadow"].forEach(f=>{r.style[f]=a[f]});let d=l.createElementNS(J,"span");d.style.position="absolute",d.style.visibility="hidden",d.style.whiteSpace="pre",d.style.pointerEvents="none",["fontFamily","fontSize","fontWeight","fontStyle","letterSpacing"].forEach(f=>{d.style[f]=a[f]});let u=2,m=8,w=()=>{d.textContent=r.value??"";let f=Math.ceil(d.getBoundingClientRect().width)+u;r.style.width=`${Math.max(f,m)}px`};r.style.boxSizing="content-box",r.style.flex="0 0 auto",t.classList.add("zen-tidy-tabs-inline-editing"),l.documentElement.classList.add("zen-tidy-tabs-editing"),t.style.display="none",t.parentNode.insertBefore(r,t),r.parentNode.insertBefore(d,r),w();let x=!1,E=f=>{if(x)return;x=!0;let P=r.value.trim();if(S.finishInline(),f&&P&&P!==o)try{j(e,P),i.user.info(`Renamed group "${o}" to "${P}".`)}catch(Y){i.user.error(`Failed to rename group "${o}" to "${P}".`,Y)}},v=f=>{f.key==="Enter"?(f.preventDefault(),E(!0)):f.key==="Escape"&&(f.preventDefault(),E(!1)),f.stopPropagation()},T=()=>E(!0),R=f=>{f.target===r||r.contains(f.target)||E(!0)};r.addEventListener("input",w),r.addEventListener("keydown",v,!0),r.addEventListener("blur",T),l.addEventListener("mousedown",R,!0),S.active={input:r,labelEl:t,group:e,original:o,discard:()=>E(!1),cleanup:()=>{d.remove(),r.removeEventListener("input",w),r.removeEventListener("keydown",v,!0),r.removeEventListener("blur",T),l.removeEventListener("mousedown",R,!0)}},r.focus(),r.select()},finishInline(){let e=S.active;e&&(S.active=null,e.cleanup?.(),e.input.remove(),l.documentElement.classList.remove("zen-tidy-tabs-editing"),e.labelEl.style.removeProperty("display"),e.labelEl.classList.remove("zen-tidy-tabs-inline-editing"))},cancelInline(){S.active?.discard?.()}};var V=()=>{if(i.init.info("Loading Zen Tidy Tabs…"),i.init.debug("location:",(()=>{try{return location.href}catch{return"?"}})()),i.init.debug(`Environment: gBrowser.addTabGroup is ${typeof p.addTabGroup}, ${p.tabs.length} tab(s) open.`),O.inject(),S.install(),k.installEmptyWatcher(),n.debug&&A.run(),g.__zenTidyTabsMountRetryTimer&&(clearInterval(g.__zenTidyTabsMountRetryTimer),g.__zenTidyTabsMountRetryTimer=null),!$.mount()){let e=0;g.__zenTidyTabsMountRetryTimer=setInterval(()=>{($.mount()||++e>n.timing.mountMaxAttempts)&&(clearInterval(g.__zenTidyTabsMountRetryTimer),g.__zenTidyTabsMountRetryTimer=null,l.getElementById(n.ui.controlId)||i.dom.warn(`Tidy control not placed after ${e} attempt(s); it will appear when you hover the tab separator.`))},n.timing.mountRetryMs)}g.zenTidyTabs={run:()=>z.runTidy(),settings:()=>_.settings(),mount:()=>$.mount(),diagnose:()=>A.run(),injectStyles:()=>O.inject(),collect:(e=!0)=>I.collect(e)},i.init.info("Ready — left-click the Tidy control to organize tabs; right-click it for settings.")};V();})();
